...
async function handleRemove() {
  const ok = await confirm({
    ...
    message: t('members.removeConfirm', { name: props.member.name }),
...
<template>
  <BaseModal :open="true" size="sm" title-id="member-detail-title" @close="emit('close')">
    <div class="flex items-start justify-between mb-5">
      <h2 id="member-detail-title" class="text-lg font-semibold text-gray-900">
